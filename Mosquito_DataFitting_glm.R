...
mfE_seq <- seq(min(x), max(x), length.out = max(x))
...
l2 <- coef(summary(fit))["x", "Estimate"] # also equals to beta_1
...
x_new <- seq(0, 10, length.out = length(upper_bound)) #change min(x), max(x) to 0 and 10
